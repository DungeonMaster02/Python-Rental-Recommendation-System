...
def combine(datafile1,datafile2):
    ...
    data.to_csv(PROCESSED_DIR / "Crime_Data_from_2010_to_2024.csv", index=False)

def get_monthly():
    conn = connection.get_connect()
    crime_df = pd.read_sql("SELECT crime_id, latitude, longitude, date, type FROM crime", conn)
...
if __name__=="__main__":
    combined = PROCESSED_DIR / "Crime_Data_from_2010_to_2024.csv"
    if not combined.exists():
        datafile1 = glob.glob(str(RAW_DIR / "Crime_Data_from_2010_to_2019*.csv"))
        datafile2 = glob.glob(str(RAW_DIR / "Crime_Data_from_2020_to_2024*.csv"))
