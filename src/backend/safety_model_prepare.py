...
def get_grid() -> gpd.GeoDataFrame:
    ...
    grid = gpd.read_file(PROCESSED_DIR / "LA_400m_grid" / "LA_400m_grid.shp").to_crs("EPSG:32611").reset_index(drop=True)
...
def get_monthly_panel() -> pd.DataFrame:
    ...
    try:
        rows = dbe.query('monthly_crime',['grid_id', 'month_start', 'crime_count', 'property_crime', 'violence_crime'])
    finally:
        conn.close()
    panel = pd.DataFrame(rows, columns=['grid_id', 'month_start', 'crime_count', 'property_crime', 'violence_crime'])
...
def get_building_feature() -> pd.DataFrame:
    ...
    grid = get_grid()
    buildings = gpd.read_file(RAW_DIR / "Building_Footprints-shp" / "building.shp").to_crs(grid.crs)
...
def get_osm_raw(grid: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    final_cache = RAW_DIR / "osm_raw_buffer400.gpkg"
    chunk_dir = RAW_DIR / "osm_chunks"
