...
from path_config import ENV_FILE, PROCESSED_DIR, RAW_DIR, RESULTS_DIR, USC_CAMPUS_DIR
...
def ensure_inputs():
    ...
    building_shp = RAW_DIR / "Building_Footprints-shp" / "building.shp"
    city_boundary_shp = RAW_DIR / "City_Boundary" / "City_Boundary.shp"
    usc_campus = USC_CAMPUS_DIR / "usc_campus.shp"
    crime_data1 = glob.glob(str(RAW_DIR / "Crime_Data_from_2010_to_2019*.csv"))
    crime_data2 = glob.glob(str(RAW_DIR / "Crime_Data_from_2020_to_2024*.csv"))
    # or
    combined_crime = PROCESSED_DIR / "Crime_Data_from_2010_to_2024.csv"
...
def check_stage():
    ...
    grid_shp = data_dir / "LA_400m_grid" / "LA_400m_grid.shp"
...
def data_pipeline():
    ...
    while check_stage():
        ...
        if stage == "grid":
            ...
            subprocess.run([sys.executable, str(BACKEND_DIR / "map_division.py")], check=True)
        elif stage == "crime":
            print("Missing combined crime data, running crime_data_processing.py ...")
            subprocess.run([sys.executable, str(BACKEND_DIR / "crime_data_processing.py")], check=True)

        elif stage == "prepare":
            refresh_crime_tables()
            print("Missing monthly panel data, running safety_model_prepare.py ...")
            subprocess.run([sys.executable, str(BACKEND_DIR / "safety_model_prepare.py")], check=True)

        elif stage == "safety":
            print("Missing safety model outputs, running safety_main.py ...")
            subprocess.run([sys.executable, str(BACKEND_DIR / "safety_main.py")], check=True)
